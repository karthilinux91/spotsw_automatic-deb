...
# 
#   DESCRIPTION: Discover the software packages and version  on debian & ubuntu
...
######################################################
echo ""
lsb_release -d  >$mainpath/tempfiles/1
check="$(echo $? )"
#echo $check 
if [ $check -eq 0 ]
then
	#echo "option 1"
        ...
        flag_version=`lsb_release -rs`
	flag_version=`printf "%.0f\n" "$flag_version"`
        echo Flag_version: $flag_version  

else
	#echo "option 2"
        cat /etc/os-release 2&1>$mainpath/tempfiles/1	
	if [ $check -eq 0 ]
	then 
	#echo "option 3"
...
if [ "$flag_version" = "17" ] ||  [ "$flag_version" = "16" ]
then 
   flag_version=9 

elif [ "$flag_version" = "15" ] ||  [ "$flag_version" = "14" ]
then 
   flag_version=8

elif [ "$flag_version" = "13" ] ||  [ "$flag_version" = "12" ] 
then 
   flag_version=7
fi 
...
elif [ "$flag" = "debian8" ] || [ "$flag" = "debian7"  ]
then 
...
if [ "$flag" = "debian9"  ]  || [ "$flag" = "debian8" ] || [ "$flag" = "debian7"  ]
then
echo "osdetail:" ` cat /etc/lsb-release  | awk -F"DISTRIB_DESCRIPTION=" '{print $2}' | xargs` 
echo ""\"osdetail"\" ":"\"` cat /etc/lsb-release  | awk -F"DISTRIB_DESCRIPTION=" '{print $2}' | xargs`"\" "," >> report.json
fi 
...
if [ "$flag" = "rhel7"  ]  || [ "$flag" = "rhel6" ] || [ "$flag" = "rhel5"  ]
then
...
if [ "$flag" = "debian9"  ]  || [ "$flag" = "debian8" ] || [ "$flag" = "debian7"  ]
then
echo  "	<td class=""text-left"">`cat /etc/lsb-release  | awk -F"DISTRIB_DESCRIPTION=" '{print $2}' | xargs`</td>">>  report.html
fi 
...
echo "------"
> $mainpath/tempfiles/store.txt
$mainpath/packages/locate$flag_version -r /apache2ctl*$ -d $mainpath/mlocate.db | xargs file  2>&1 | grep executable  | awk -F":" '{print $1}' > $mainpath/tempfiles/store.txt
...
then
	...
	for (( c=1; c<=$total; c++ ))
	do
	   version="`${array[c]} -v  2>&1 | grep cli | awk '{print $1 $2}' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?+([.][0-9]+)?'`"
	   path=${array[c]}
...
echo "------"
> $mainpath/tempfiles/store.txt
$mainpath/packages/locate$flag_version -r /version.sh$ -d $mainpath/mlocate.db | xargs file 2>&1 | grep executable | awk -F":" '{print $1}' > $mainpath/tempfiles/store.txt
FILENAME="$mainpath/tempfiles/store.txt"
...
then
	...
	for (( c=1; c<=$total; c++ ))
	do
	   version="`${array[c]}  2>&1 | grep number  | awk '{print $3}'`" 	
	   path=${array[c]}
	   echo "Version : " $version
	   echo "path:" $path
	   echo ""
...
>$mainpath/tempfiles/store.txt
$mainpath/packages/locate$flag_version -r /bin/postgres$ -d $mainpath/mlocate.db| xargs file 2>&1 | grep dynamically | awk -F":" '{print $1}' > $mainpath/tempfiles/store.txt
FILENAME="$mainpath/tempfiles/store.txt"
##############################################################################################
if [ -s ${FILENAME} ]
then
...
then
	...
	do
	  ...
	  then
	   ...
	   $mainpath/packages/wp-cli plugin list --format=csv  --allow-root --fields=name,version | grep -v name > $mainpath/tempfiles/wpplugins.txt 
...
echo "-------"
>$mainpath/tempfiles/store.txt
$mainpath/packages/locate$flag_version -r /bin/nodejs$  -d $mainpath/mlocate.db  | xargs file 2>&1 | grep 'executable\|dynamically' | awk -F":" '{print $1}' > $mainpath/tempfiles/store.txt
FILENAME="$mainpath/tempfiles/store.txt"
...
$mainpath/packages/locate$flag_version -r /bin/npm-cli.js*$  -d $mainpath/mlocate.db  | xargs file 2>&1 | grep 'ASCII' | awk -F":" '{print $1}' > $mainpath/tempfiles/store.txt
...
if [ "$flag" = "debian9"  ] 
then
	systemctl status ufw >$mainpath/tempfiles/store.txt
	check="$(echo $? )"
	if [ $check -eq 0 ]
	then
	status=Active
...
echo "------"
> $mainpath/tempfiles/store.txt
cat $mainpath/tempfiles/process | awk '{print $11}' | grep apache* | uniq > $mainpath/tempfiles/store.txt
...
echo "<font color="blue">Version:</font>&nbsp;&nbsp;$version<hr><font color="blue">Path:</font>&nbsp;&nbsp;$path ">>report.html
  ...
  echo "Apache httpd not found!!"
  echo "\"apache-run"\":["\"not detected"\"], >> report.json 
...
then
  ...
  echo "Java not found !"
  echo "\"java-run"\":["\"not detected"\"], >> report.json 
...
then
  ...
  echo "PostgreSQl_Server not found!"
  echo "\"postgresql_server-run"\":["\"not detected"\"], >> report.json 
...
#####################################################################################
# system opened ports in system 						    #
#####################################################################################

echo "Open Ports in system "
echo "----------" 
>$mainpath/tempfiles/store.txt
...
$mainpath/packages/nmap9 localhost | grep open | sed 's/ //g'  | sed 's/tcp //g' | exec sed 's/tcpopen//g' >$mainpath/tempfiles/store.txt
#else 
...
$mainpath/packages/nmap8 localhost | grep open | sed 's/ //g'  | sed 's/tcp //g' | exec sed 's/tcpopen//g' >$mainpath/tempfiles/store.txt
#else 
#iptables -L -n  | awk -F "dpt:" '{print $2}'  | grep -o -E '[0-9]+' >$mainpath/tempfiles/store.txt
fi 
...
$mainpath/packages/nmap7 localhost | grep open | sed 's/ //g'  | sed 's/tcp //g' | exec sed 's/tcpopen//g' >$mainpath/tempfiles/store.txt
#else 
#iptables -L -n  | awk -F "dpt:" '{print $2}'  | grep -o -E '[0-9]+' >$mainpath/tempfiles/store.txt
fi 
...
then
    echo  " <tr>                                            "       >>  report.html
    echo  " <td class=""text-left"">Opened Ports in system </td>      "       >>  report.html
    echo  " <td class=""text-left""><br>		    "  	    >>  report.html
